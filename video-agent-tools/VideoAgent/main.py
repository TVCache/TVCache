...
def ReActAgent(video_path, question, base_dir='preprocess', vqa_tool='videollava', use_reid=True, openai_api_key='your_openai_api_key'):
    ...
    def object_memory_querying(question):
        ...
        with open('prompts/database_query_prompt.txt') as f:
            t = f.read()
        prompt.template = t
        llm = ChatOpenAI(model='gpt-4', temperature=0.0, openai_api_key=openai_api_key, base_url="")
...
def preprocess(video_path_list, base_dir='preprocess', show_tracking=False):
    ...
    if captioning_model == None:
        captioning_model = Captioning(video_path_list=preprocess_list, 
                            base_dir=base_dir)
        
    captioning = captioning_model
    captioning.video_path_list = preprocess_list
    captioning.base_dir = base_dir
    captioning.run()
...
def main(video_path_list, video_question_list, base_dir='preprocess', vqa_tool='videollava', use_reid=True, openai_api_key='your_openai_api_key'):
    ...
    for i in range(question_num):
       preprocess(video_path_list=[video_path_list[i]], 
               base_dir=base_dir, 
               show_tracking=False)
...
if __name__ == '__main__':
    ...
    video_question_list = [
        ...
        """Analyze the importance and context of c's tea-drinking moments in the video. what could be their purpose?  Just respond with the option number, do not say option x, just say x in your response.", "option 0": "C's tea-drinking moments in the video were important because they were a way for c to stay hydrated. c drank a lot of tea throughout the video, and it helped them to stay energized.", "option 1": "C's tea-drinking moments in the video were important because they were a way for c to socialize. c drank tea with a friend at the beginning of the video, and it helped them to connect with each other.", "option 2": "C's tea-drinking moments in the video were important because they were a way for c to take a break from the sewing. c seemed to enjoy drinking tea, and it helped them to relax and focus on the task at hand.", "option 3": "C's tea-drinking moments in the video were important because they were a way for c to mark the progress of the sewing. c drank tea at the beginning, middle, and end of the video, and it helped them to track their progress.", "option 4": "C's tea-drinking moments in the video were important because they provided breaks from the sewing. c seemed to enjoy drinking tea, and it helped them to relax and focus on the task at hand."""
    ]
