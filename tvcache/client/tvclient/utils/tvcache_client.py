"""TVCacheClient for interacting with the TVCache HTTP server."""
...
class TVCacheClient:
    """Client for interacting with the TVCache HTTP server."""

    def __init__(self, base_url: str = "http://localhost:8000"):
        """Initialize the TVCache client.
    ...
    def _headers(self, task_name: Optional[str] = "__admin__") -> Dict[str, str]:
        return {"X-Task-Name": task_name or "__admin__"}

    # task-specific
    def exact_match(self, task_name: str, tool_calls: List[str]) -> bool:
        ...
        try:
            response = self.session.get(
                ...
                headers=self._headers(task_name),
            )
    ...
    # task-specific
    def get(self, task_name: str, tool_calls: List[str]) -> Tuple[Optional[str], Optional[Any], Optional[float]]:
        ...
        try:
            ...
            return True, data["env_id"], data["value"], data.get("tool_exec_time", 0)

        except (requests.RequestException, ValueError, TypeError):
            return False, None, None, 0.0

    # task-specific
    def intel_prefix_match(self, task_name: str, tool_calls: List[Any]) -> Tuple[Optional[str], Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
    ...
    # task-specific 
    def prefix_match(self, task_name: str, tool_calls: List[Any]) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
        """Check for a prefix match and return environment ID and history if found.
    ...
    # task-specific
    def mark_stateless(self, task_name: str, history: List[str], env_id: str) -> bool:
    ...
    # task-specific  
    def put(self, task_name: str, history: List[str], env_id: str, values: List[str] = None, tool_exec_times: List[float] = None, start_idx: int = 0) -> List[str]:
    ...
    # not implemented
    def remove(self, task_name: str, history: List[str]) -> bool:
    ...
    # not implemented
    def can_extend(self, task_name: str, history: List[str], suffix: List[str]) -> Tuple[bool, Optional[List[str]]]:
    ...
    # task-specific
    def get_test_result(self, task_name: str, tool_calls: List[str]) -> Tuple[bool, Optional[str]]:
    ...
    # task-specific
    def unref(self, env_id: str, task_name: Optional[str] = None) -> bool:
